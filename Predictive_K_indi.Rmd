...
for(i in 1:k) {
  ...
  train.glm.noint <- survey::svyglm(formula = formula02, design = train.des1, 
                                    na.action = na.omit, family = quasibinomial)
  train.tree.noint<-rpart::rpart(formula=formula02,data = Final_data,weights =hhwgt,method = "class")
  train.random.noint<-randomForest(formula=formula02,weights=Final_data$hhwgt,data=Final_data,mtry=4,ntree=3000)

  
  FinaL_new<- Final_data %>% select(K_in, urbrural, F_15_49, M_1_15,
                             M_15_49, F_1_15, F_GE50, M_GE50, Sex_hh, Age_hh, cost, s7aq06, brick, mud, wood,
                             electricity, agri, non_agri, daylobour, salarywage,safety_nate) 

Final <- FinaL_new[complete.cases(FinaL_new),]
m_xrf <- xrf(K_in ~ ., Final, family = 'binomial', 
 ...
 pred.rul <- predict(m_xrf,test.set,type = "class")
  ...
  tab.noInt <- table(Predicted = test.set.noInt.pred, Observed = test.set$K_in)
  
  conf.mat.noInt <- caret::confusionMatrix(tab.noInt)
  conf.mat.noInt1<-caret::confusionMatrix(pred.tr,test.set$K_in)
  conf.mat.noInt2<-caret::confusionMatrix(pred.random,test.set$K_in)
  tab.noInt3 <- table(Predicted = pred.rul, Observed = test.set$K_in)
